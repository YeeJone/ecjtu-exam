...
class Exam_manage extends CI_Model
{
	...
	{
		$query = $this->db->insert('examlist',$data);
		if ($query) {
			return true;
		}else{
			return false;
	...
	{
		$data = null;
		$this->db->select('id,examname,examlist,starttime,examinfo,teachername,state');
		...
		foreach ($query as $key => $value) {
			$data = array(
				...
				'E_list' => $this->transList($value['examlist']),
				'E_info' => $value['examinfo'],
				'E_starttime' => $value['starttime'],
				'E_teacher' => $this->getTechName($value['teachername']),
				'E_status' => $value['state'],
				'edit' => $value['id']
				);
	...
	public function updateStatus($id,$status)
	{
		$this->db->where('id', $id);
		if ($status == 1) {
			$query = $this->db->update('examlist', array('state' => 1 ));
			if ($query) { return 1; }
			else{ return 0; }
		}else
		{
			$query = $this->db->update('examlist', array('state' => 0 ));
			if ($query) { return 1; }
			else{ return 0; }
		}
	}
