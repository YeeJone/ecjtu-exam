    ...
    <script type="text/javascript">
        ...
        window.onload = function(){
            ...
            if (flash_files==null) {
                alert('你已经完成考试了');
                $.ajax({
                    url:"<?php echo site_url('student/stutask/changeTaskStatus/'.$stuid.'/'.$taskid.'');?>",
                    type:"post",
                    success:function(data){
                        console.log(data);
                        if (data == 1) {
                            window.location.href="<?php echo site_url('student/stutask/taskList/'.$stuid.'');?>";
                        }
                    },
            ...
            stop.addEventListener('click',function(){
                ...
                recorder.getBlob(function(blob){
                    ...
                    $.ajax({
                        url: "<?php echo site_url('student/stutask/uploadRecord/'.$stuid.'/'.$taskid.'/'.$list.'/');?>"+flash_files[i-1].fileid+"",
                        type: 'post',
                        async: false,  
                        cache: false,  
                        contentType: false,  
            ...
            text_submit.onclick =function(){
                ...
                $.ajax({
                    url:"<?php echo site_url('student/stutask/uploadText/'.$stuid.'/'.$taskid.'/'.$list.'/');?>"+flash_files[i-1].fileid+"/"+flash_files[i-1].type+"",
                    type:"post",
                    data:{
                        "text":text,
                    },
            ...
            next.onclick = function() {
                if (this.innerText =='下一题') {
                    if(i<= filesLength){
                        ...
                        //console.log(s);
                        $.ajax({
                            url:"<?php echo site_url('student/stutask/updateStatus/'.$stuid.'/'.$taskid.'');?>",
                            type:"post",
                            data:{
