    ...
    <script type="text/javascript">
    	...
    	window.onload = function(){
            ...
            if (flash_files==null) {
                alert('你已经完成考试了');
                $.ajax({
                        url:"<?php echo site_url('student/stuself/deleteAssemblyStatus/'.$id.'');?>",
                        type:"post",
                        success:function(data){
                            if (data == 1) {
                                window.location.href="<?php echo site_url('student/stuself/assemblyQues/'.$stuid.'');?>";
                            }
                        },
            ...
            text_submit.onclick =function(){
                ...
                $.ajax({
                    url: "<?php echo site_url('student/stuself/uploadText/'.$stuid.'/'.$id.'/'.$name.'/');?>"+flash_files[i-1].fileid+"",
                    type:"post",
                    data:{
                        "text":text,
                    },
            ...
            next.onclick = function() {
                if (this.innerText =='下一题') {
                    if(i<= filesLength){
                        ...
                        $.ajax({
                            url:"<?php echo site_url('student/stuself/updateStatus/'.$stuid.'/'.$id.'/'.$name.'');?>",
                            type:"post",
                            data:{
                                'type':flash_files[i-1].type,
                                "file":flash_files[i-1].fileid,
                                "score":s,
                            },
                            success:function(data){
                                console.log(data);
