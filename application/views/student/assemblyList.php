...
<script type="text/javascript">
   $('#example').removeClass( 'display' ).addClass('table table-sthiped table-bordered');
    ...
    function initTable(){
        ...
        "ajax":{
            "type":"POST",
            "url":"<?php echo site_url('student/stuself/getList/'.$studentname.'');?>",
        ...
        "columns":[
            ...
            },
            {
                ...
                "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                    if(sData.status == 2){
                        $(nTd).html("<a href='<?php echo site_url('student/stuself/goAssembly/'.$studentname.'/');?>"+sData.id+"'>开始答题</a>");
                    }else if(sData.status == 0){
                        $(nTd).html("<a href='<?php echo site_url('student/stuself/goAssembly/'.$studentname.'/');?>"+sData.id+"'>继续答题</a>");
                    }else{
                        $(nTd).html("已完成");
                    }
                }
